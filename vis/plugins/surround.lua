...
vis.events.subscribe(vis.events.INIT, function()
	...
	local vis_pairs = package.loaded["pairs"] or package.loaded["vis-pairs"]
	if vis_pairs then
		get_pair = function(key, pos) return vis_pairs.get_pair(key, vis.win, pos) end
		outer = function(key) vis_pairs.key = key return vis_pairs.textobject.outer end
	end
end)
